#!/usr/bin/env bash
# awsVPN  automated installer for Debian, Ubuntu and CentOS
...
newclient () {
	...
	echo "</key>"
	echo "<tls-crypt>"
	sed -ne '/BEGIN awsVPN Static key/,$ p' /etc/openvpn/tc.key
	echo "</tls-crypt>"
	} > ~/"$1".ovpn
}
...
# CRL is read with each client connection, when awsVPN is dropped to nobody
...
# Allow traffic from awsVPN client to eth0
